...
from app.schemas import CreateProductIn, UpdateProductIn, ProductSchema
...
async def create_product(service: Annotated[ProductService, Depends()],
                         product: CreateProductIn):
    return await service.create_product(product)


@router.get('/{category_slug}', response_model=list[ProductSchema])
async def product_by_category(service: Annotated[ProductService, Depends()],
                              category_slug: str):
...
async def product_detail(service: Annotated[ProductService, Depends()],
                         product_slug: str):
    return await service.get_product_by_slug(product_slug)


@router.patch('/{product_id}', response_model=ProductSchema)
async def update_product(service: Annotated[ProductService, Depends()],
                         product_id: int,
...
async def delete_product(service: Annotated[ProductService, Depends()],
                         product_id: int):
