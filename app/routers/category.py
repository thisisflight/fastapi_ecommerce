...
from app.schemas import CategorySchema, CreateCategoryIn, UpdateCategoryIn
...
async def create_category(service: Annotated[CategoryService, Depends()], category: CreateCategoryIn):
...
async def update_category(service: Annotated[CategoryService, Depends()], category_id: int, category: UpdateCategoryIn):
    return await service.update_category(category_id, category)


@router.delete("/{category_id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_category(service: Annotated[CategoryService, Depends()], category_id: int):
